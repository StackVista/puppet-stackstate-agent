...
begin
  ...
  Puppet.info "You need the `dogapi` gem to use the StackState report (run puppet with puppet_run_reports on your master)"
end

Puppet::Reports.register_report(:stackstate_reports) do

  configfile = "/etc/sts-agent/stackstate.yaml"
  raise(Puppet::ParseError, "StackState report config file #{configfile} not readable") unless File.readable?(configfile)
  config = YAML.load_file(configfile)
  API_KEY = config[:stackstate_api_key]
  ...
  desc <<-DESC
  Send notification of metrics to StackState
  DESC
  ...
  def process
    ...
    Puppet.debug "Sending metrics for #{@msg_host} to StackState"
    ...
    Puppet.debug "Sending events for #{@msg_host} to StackState"
