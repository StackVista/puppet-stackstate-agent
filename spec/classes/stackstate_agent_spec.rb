...
describe 'stackstate_agent' do
  context 'unsupported operating system' do
    describe 'stackstate_agent class without any parameters on Solaris/Nexenta' do
      let(:facts) do
        {
          osfamily:         'Solaris',
  ...
  context 'all supported operating systems' do
    ALL_OS.each do |operatingsystem|
      describe "stackstate_agent class common actions on #{operatingsystem}" do
        ...
        it { should contain_class('stackstate_agent') }

        describe 'stackstate_agent imports the default params' do
          it { should contain_class('stackstate_agent::params') }
        end

        it { should contain_file('/etc/sts-agent') }
        it { should contain_concat('/etc/sts-agent/stackstate.conf') }
        it { should contain_file('/etc/sts-agent/conf.d').with_ensure('directory') }

        it { should contain_class('stackstate_agent::reports') }

        describe 'parameter check' do
            context 'with defaults' do
                context 'for proxy' do
                    it { should contain_concat__fragment('stackstate header').with(
                    'content' => /^dd_url: https:\/\/app.stackstate.com\n/,
                    )}
                    it { should contain_concat__fragment('stackstate header').with(
                    'content' => /^# proxy_host:\n/,
                    )}
                ...
                context 'for general' do
                    ...
                    it { should contain_concat__fragment('stackstate footer').with(
                    ...
                    it { should contain_concat__fragment('stackstate footer').with(
                    'content' => /^# additional_checksd: \/etc\/sts-agent\/checks.d\/\n/,
                    )}
                    it { should contain_concat__fragment('stackstate footer').with(
                ...
                context 'for logging' do
                    ...
                    it { should contain_concat__fragment('stackstate footer').with(
                    'content' => /^# collector_log_file: \/var\/log\/stackstate\/collector.log\n/,
                    )}
                    it { should contain_concat__fragment('stackstate footer').with(
                    'content' => /^# forwarder_log_file: \/var\/log\/stackstate\/forwarder.log\n/,
                    )}
                    it { should contain_concat__fragment('stackstate footer').with(
                    'content' => /^# dogstatsd_log_file: \/var\/log\/stackstate\/dogstatsd.log\n/,
                    )}
                    it { should contain_concat__fragment('stackstate footer').with(
                    #'content' => /^# pup_log_file:        \/var\/log\/stackstate\/pup.log\n/,
                    )}
                    it { should contain_concat__fragment('stackstate footer').with(
                    'content' => /^# syslog_host:\n/,
                    )}
                ...
                context 'for service_discovery' do
                    it { should contain_concat__fragment('stackstate footer').without(
            ...
            context 'with a custom dd_url' do
                let(:params) {{:dd_url => 'https://notaurl.stackstate.com'}}
                it { should contain_concat__fragment('stackstate header').with(
                    'content' => /^dd_url: https:\/\/notaurl.stackstate.com\n/,
                )}
            end
            ...
            context 'with configuration for a custom checks.d' do
                let(:params) {{:additional_checksd => '/etc/sts-agent/checks_custom.d' }}
                it { should contain_concat__fragment('stackstate footer').with(
                    'content' => /^additional_checksd: \/etc\/sts-agent\/checks_custom.d\n/,
                )}
            end
            context 'with configuration for a custom checks.d' do
                let(:params) {{:additional_checksd => '/etc/sts-agent/checks_custom.d' }}
            ...
            context 'with apm_enabled set to true and env specified' do
                ...
                it { should contain_concat__fragment('stackstate apm footer').with(
                    'content' => /^\[trace.agent\]\n/,
                )}
                it { should contain_concat__fragment('stackstate apm footer').with(
                    'content' => /^env: foo\n/,
        ...
        if DEBIAN_OS.include?(operatingsystem)
          it { should contain_class('stackstate_agent::ubuntu') }
        elsif REDHAT_OS.include?(operatingsystem)
          it { should contain_class('stackstate_agent::redhat') }
        end
      end
    end
  end
  ...
  context "with facts to tags set" do
    describe "ensure facts_array outputs a list of tags" do
      ...
      let(:facts) do
        ...
        it { should contain_concat('/etc/sts-agent/stackstate.conf') }
        it { should contain_concat__fragment('stackstate tags').with(/tags: osfamily:redhat, facts_array:one, facts_array:two, facts_array:three/) }
      end
    end
  end
end
